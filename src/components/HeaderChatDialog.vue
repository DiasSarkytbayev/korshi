...
function connect(event) {
  const socket = new SockJS(`${import.meta.env.VITE_BASE_API_URL}app`);
...
function onConnected() {
  stompClient.value.subscribe(`/user/${userData.value.id}/queue/messages`, onMessageReceived);
...
async function fetchAndDisplayUserChat() {
  const userChatResponse = await fetch(`/messages/${userData.value.id}/${selectedUserId.value}`);
  userChat.value = await userChatResponse.json();
}

function onError() {
  const res = 'Could not connect to WebSocket server. Please refresh this page to try again!';
}
...
function onLogout() {
  stompClient.value.send(
    ...
    {},
    JSON.stringify({ userId: userData.value.id, fullName: fullName.value, status: 'OFFLINE' })
  );
